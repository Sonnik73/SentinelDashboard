...
- Виджет `cameras` реализован (живой поток из RTSP через постоянный процесс `ffmpeg`, частота кадров — `fps` per-камера, задаётся числом в Settings, по умолчанию 2, диапазон 1-10 — снижено с изначальных глобальных 3 после того, как реальный Pi не тянул такую частоту, см. `docs/MODULES.md`), но RTSP-путь (`path` в конфиге, по умолчанию `/1/1`) — типичная конвенция Tiandy, **не проверена вживую** на реальных камерах. Учётные данные задаются либо в Settings (раздел «Учётные данные камер»), либо через `CAMERA_USERNAME`/`CAMERA_PASSWORD`; введённое в UI пишется в `config/local.json` (`core/secrets.py`, права `0600`) и имеет приоритет над переменной окружения. В `config/dashboard.json` пароль не попадает никогда — этот файл в git.
