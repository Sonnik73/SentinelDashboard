...
- Погода переведена на Yandex Weather API (с v1.5.0, ключ через `YANDEX_WEATHER_API_KEY`, троттлинг на сервере — см. `docs/MODULES.md`), но **не проверена вживую** — сеть этой среды разработки блокирует `api.weather.yandex.ru` так же, как раньше блокировала `api.open-meteo.com`. Нужен первый реальный запуск на боевом окружении, чтобы подтвердить формат ответа API.
