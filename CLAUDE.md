...
- Битый `config/dashboard.json` не роняет сервер (`core/module_api.py` изолирует ошибки импорта по модулям), но `/api/widgets` продолжает показывать неисправные виджеты как `"enabled": true` — известный пробел, см. `docs/ROADMAP.md`.
