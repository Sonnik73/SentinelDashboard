...
- Виджет `cameras` реализован (снимок из RTSP через `ffmpeg`, см. `docs/MODULES.md`), но RTSP-путь (`path` в конфиге, по умолчанию `/1/1`) — типичная конвенция Tiandy, **не проверена вживую** на реальных камерах (приедут позже). Учётные данные — через `CAMERA_USERNAME`/`CAMERA_PASSWORD`, не коммитятся.
