...
- Виджет `cameras` реализован (живой поток из RTSP через постоянный процесс `ffmpeg`, частота кадров — `FRAME_RATE` в `modules/cameras/service.py`, по умолчанию 2 кадра/сек — снижено с 3 после того, как реальный Pi не тянул такую частоту, см. `docs/MODULES.md`), но RTSP-путь (`path` в конфиге, по умолчанию `/1/1`) — типичная конвенция Tiandy, **не проверена вживую** на реальных камерах. Учётные данные — через `CAMERA_USERNAME`/`CAMERA_PASSWORD`, не коммитятся.
