...
- [x] Error handling — verified: missing data/birthdays.json returns `{"source": "local", "items": []}` without crashing; unreachable network hosts return `{"online": false, "ping_ms": null}` without crashing. An invalid `config/dashboard.json` doesn't crash the server (`core/module_api.py` isolates per-module import failures with a `[WARN]` log); as of v2.7.1 the failure is also tracked (`core/module_api.py`'s `MODULE_STATUS`) and surfaced as `"available": false` + `"error"` on `/api/widgets`/`/api/views`, so Settings shows the broken widget struck through with a ⚠️ tooltip instead of a normal widget that silently hangs on "Loading..." forever
- [x] **Weather provider decided, implemented, and confirmed working on real hardware.** `api.open-meteo.com` was unreachable from the deployment network (v1.3.3). Switched to Yandex Weather API in v1.5.0 — then the project owner discovered Yandex had quietly dropped its permanent free "on your site" tier down to a 14-day trial, invalidating that decision before it ever ran in production. Re-decided in v1.6.0: switched to scraping rp5.ru instead (no quota, no key, no trial expiry — the tradeoff is depending on undocumented page structure instead of a documented API). Two rounds of real-hardware testing on the actual Pi (v1.6.1, v1.6.2) found and fixed three real bugs that no amount of mocked/sandboxed testing could have caught: Moscow and Krasnodar's URLs were multi-station "hub" pages with no forecast table (both fixed to the specific station URL), and the humidity/temperature regexes were too strict for a real page variant with tooltip attributes on the data cell. **Confirmed 03.07.2026:** `curl /api/weather` on the real Pi returns `"source": "online"` with real, plausible numbers for all three cities (Москва 26°C/44%, Краснодар 29°C/57%, Ульяновск 30°C/35%)
