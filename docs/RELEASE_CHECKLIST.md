...
- [x] **Weather provider decided and implemented:** `api.open-meteo.com` was unreachable from the deployment network (see v1.3.3 investigation). Project owner chose Yandex Weather API over rp5.ru (rp5 would've meant either a fragile HTML scrape or manual approval for its paid XML export). Implemented in v1.5.0: `modules/weather/service.py` calls Yandex's `v2/informers` endpoint with an API key from `YANDEX_WEATHER_API_KEY`, and throttles itself server-side to stay under the free tier's 50 requests/day cap (won't fetch live more than once per 2h regardless of how many clients poll `/api/weather`). **Not verified against the live endpoint** — this development environment's network policy blocks `api.weather.yandex.ru` the same way it blocked open-meteo, so the request/header/response-shape logic was verified with a mocked HTTP client instead of a real call. Needs a first real run on the actual deployment (with the API key set) to confirm it actually works end-to-end
