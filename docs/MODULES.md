...
- Data source: a genuine live feed from each camera's RTSP stream, not a periodic snapshot. `modules/cameras/service.py` keeps one persistent `ffmpeg` process per camera running continuously (`ffmpeg -rtsp_transport tcp -i <rtsp-url> -r <FRAME_RATE> -f mjpeg -`), streaming concatenated JPEG frames to stdout. A one-shot `ffmpeg` process per request (the original v2.4.0 design) can't sustain multiple fps — an RTSP handshake alone can take 1-2s. `FRAME_RATE` defaults to 2 (lowered from an initial 3 in v2.8.2 — the real Raspberry Pi couldn't keep up); `modules/cameras/widget.js`'s `CAMERA_FRAME_INTERVAL_MS` must be kept in sync with it (`1000 / FRAME_RATE`, in milliseconds)
...
- Configured in `config/dashboard.json` under `cameras.hosts`, each entry needs `id` (used in the API URL and cache filename), `name` (display label), `ip`, and optionally `port` (default 554), `path` (default `/1/1`), `quality` (ffmpeg `-q:v`, default 5 — 2 best/largest, 31 worst/smallest, exposed as a "high/medium/low/minimum" dropdown), and `resolution` (passed to ffmpeg's `-vf scale=`, default empty = camera's native resolution, exposed as a dropdown of common sizes) — the RTSP path is camera-model-specific; `/1/1` is a common Tiandy convention but **unverified beyond a specific model/firmware**, confirm it works once the camera is on the network and adjust `path` if not. Changing quality/resolution through Settings restarts that camera's stream on the next request so the change takes effect immediately
