...
- Fullscreen Swipe: while a widget is fullscreen, swiping left/right jumps to the next/previous widget in the current view (wraps at the ends) — `static/js/ui.js`'s `swipeToWidget()` switches the fullscreen target directly rather than exiting first, which also avoids losing the "direct user gesture" browsers require to grant fullscreen. Guards against a vertical-dominant touch move (scrolling a widget's own long content) being misread as a swipe
